...
function mtkwifi.__setup_vifs(cfgs, devname, mainidx, subidx)
    ...
    for j=1,tonumber(cfgs.BssidNum) do
        ...
        vifs[j].__temp_ssid = mtkwifi.read_pipe("iwconfig "..vifs[j].vifname.." | grep ESSID | cut -d \'\"\' -f 2")
        ...
        if (vifs[j].__temp_ssid ~= "") then
            vifs[j].__ssid = vifs[j].__temp_ssid
        else
            vifs[j].__ssid = cfgs["SSID"..j]
        end
    ...
    for _,line in ipairs(mtkwifi.__lines(content)) do
        ...
        if string.byte(line) ~= string.byte("#") then
            ...
            if band then
                ...
                local ssid = string.gsub(bssLineStr, "%s%S+ %S+ %S+ %d %d %S+", "")
                tmp[alMac][band][bssInfoIdx]['ssid'] = ssid
                local newBssLineStr = string.match(bssLineStr, "%S+ %S+ %S+ %d %d %S+")
                for token in string.gmatch(newBssLineStr, "(%S+)%s?") do
                    tokIdx = tokIdx + 1
                    if tokIdx == 1 then
                        tmp[alMac][band][bssInfoIdx]['authMode'] = token
                    elseif tokIdx == 2 then
                        tmp[alMac][band][bssInfoIdx]['encType'] = token
                    elseif tokIdx == 3 then
                        tmp[alMac][band][bssInfoIdx]['passPhrase'] = token
