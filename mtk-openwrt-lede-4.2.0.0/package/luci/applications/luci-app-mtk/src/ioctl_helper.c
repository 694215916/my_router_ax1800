...
static unsigned int get_w_mode(char *interface)
{
	int socket_id;
	struct iwreq wrq;
...
int get_macaddr(lua_State *L)
{
	char *ifname = luaL_checkstring(L, 1);
	struct ifreq ifr;
	char *ptr;
	int skfd;
...
{
	char new_pin[9];
	char *interface = luaL_checkstring(L, 1);
...
int getCurrentWscProfile(lua_State *L)
{
	...
	if((data = malloc(sizeof( WSC_CONFIGURED_VALUE))) == NULL){
	...
	lua_pushstring(L, data->WscSsid);  /* push value */
	lua_settable(L, -3);
	...
	//EncrypType
	tmp_str[0] = '\0';
	getWPSEncrypType(data, tmp_str);
	lua_pushstring(L, "EncType");  /* push key */
	lua_pushstring(L, tmp_str);  /* push value */
	lua_settable(L, -3);

	lua_pushstring(L, "WscWPAKey");  /* push key */
	lua_pushstring(L, data->WscWPAKey);  /* push value */
	lua_settable(L, -3);
...
{
	int  len, i;
	char buffer[199];		// 33(characters in SSID) * 6(maximum length of a HTML entity)  = 198 + 1(null character) = 199
	char *pOut,*pBufLimit;
...
int apcli_get_wps_status(lua_State *L)
{
	...
	if (strstr(interface,"apcli") != NULL) {
		...
		if (wsc_profile != NULL) {
			...
			lua_pushstring(L, wsc_profile->Profile[profile_idx].SSID.Ssid);  /* push value */
			lua_settable(L, -3);
			...
			switch (wsc_profile->Profile[profile_idx].EncrType) {
				case 0x0002:
					...
					if (wsc_profile->Profile[profile_idx].KeyIndex == 1) {
						...
						lua_pushstring(L, wsc_profile->Profile[profile_idx].Key);  /* push value */
						lua_settable(L, -3);
