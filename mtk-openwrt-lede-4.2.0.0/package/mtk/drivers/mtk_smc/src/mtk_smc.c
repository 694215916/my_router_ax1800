...
static int mtk_smc_set_issue(void *data, u64 val)
{
	*((u64 *)data) = 0;
	...
	/* echo 1 > /sys/kernel/debug/mtk_smc/issue */
	if (val == 1)
		if (!mtk_smc_issue())
			*((u64 *)data) = 1;
...
static int __init mtk_smc_init(void)
{
	...
	/* /sys/kernel/debug/mtk_smc/ */
	mtk_smc_dir = debugfs_create_dir("mtk_smc", NULL);
	if (!mtk_smc_dir)
		goto fail;

	/* /sys/kernel/debug/mtk_smc/parameters/ */
	parameters_dir = debugfs_create_dir("parameters", mtk_smc_dir);
	if (!parameters_dir)
		goto fail;

	/* /sys/kernel/debug/mtk_smc/parameters/aX */
	...
	/* /sys/kernel/debug/mtk_smc/results/ */
	results_dir = debugfs_create_dir("results", mtk_smc_dir);
	if (!results_dir)
		goto fail;

	/* /sys/kernel/debug/mtk_smc/results/rX */
	...
	/* /sys/kernel/debug/mtk_smc/issue */
