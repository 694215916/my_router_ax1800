...
# We assume that the MAX size of MT7629 NAND image is 24 Mbytes.
declare -i FLASH_SIZE=24*1024*1024
#declare -i COUNT_NUM="$FLASH_SIZE"/"$BLOCK_SIZE"

# Generate a single image
echo "Make SPI NAND Image block size 65536 Bytes"
rm -rf $OUTPUT_NAME
# Only padding to kernel partition start - 0x1C0000
dd if=/dev/zero of=$OUTPUT_NAME bs=$BLOCK_SIZE seek=0 count=28
...
#	dd if=$KERNEL_NAME of=$OUTPUT_NAME bs=$BLOCK_SIZE seek=28 conv=notrunc
...
echo "Make SPI NAND flashimage.bin completed..."
